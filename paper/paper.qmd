...
There are several assumptions for simple linear regression. One is that the relationship between the response and predictor must be linear. The others relate to the error terms. The errors must be independently and identically distributed normal with constant variance.
...
[@fig-rmr-residual] shows the residuals scattered randomly. While there are a few oddities with extreme points, these are likely due to a lack of data rather than assumption violations. Based on [@fig-rmr-residual], the assumptions of independence, constant variance, and linearity seem satisfied.
